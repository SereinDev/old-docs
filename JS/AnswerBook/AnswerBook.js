...
function callback({ message, message_id }) {
    const answer = answers[Math.floor(Math.random() * answers.length)] || '不知道';

    const question = message.length > 5 ? `「${message.substring(5)}」` : '你';

    return `[CQ:reply,id=${message_id}]${question}的答案是:\n${answer}`;
}
