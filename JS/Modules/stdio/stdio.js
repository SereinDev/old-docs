 ...
 */
export function getFiles(path, searchPattern, searchOption = SearchOption.TopDirectoryOnly) {
    if (searchPattern)
 ...
 * @returns 指定目录中与指定的搜索模式和选项匹配的文件的完整名称（包含路径）的数组；如果未找到任何文件，则为空数组
 */
export function getDirectories(path, searchPattern, searchOption = SearchOption.TopDirectoryOnly) {
    if (searchPattern)
